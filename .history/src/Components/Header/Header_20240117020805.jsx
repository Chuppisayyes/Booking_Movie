...
export default function Header() {
  ...
  const handleNavigation = (page, index) => {
    if (page === "Quản lý") {
      // Nếu là liên kết "Quản lý", chuyển hướng đến "/manage-admin"
          ...
          <Grid container alignItems="center">
            ...
            <Grid container item justifyContent="center" alignItems="center" sx={{ display: { xs: 'none', lg: 'flex' } }} lg={5}>
              {pages.map((page, index) => (
                <a className={style.jss3} key={index} onClick={() => handleNavigation(page, index);}>
                  <Typography variant="h4" sx={{ fontSize: 14, fontWeight: 500 }}>
                    {page}
                  </Typography>
                </a>
              ))}
            ...
            <Drawer
              ...
              <List sx={{ flexGrow: 1 }}>
                {pages.map((page, index) => (
                  <ListItem sx={{ paddingTop: '22px' }} key={index}>
                    <a className={style.jss3} onClick={() => handleNavigation(page, index)}>
                      <Typography variant="h4" sx={{ fontSize: 14, fontWeight: 500 }}>
                        {page}
                      </Typography>
                    </a>
