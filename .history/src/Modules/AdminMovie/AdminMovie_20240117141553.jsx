...
import { deleteMovie, getMoviesSearch, addMovie } from '../../Apis/movieApi';
...
export default function AdminMovie() {
    ...
    const handleChangePage = (page) => {
        setSearchParams({ soTrang: page });
    };

    useEffect(() => {
        const page = searchParams.get('soTrang') || 1;
        dispatch(getMovieList({ page, tenPhim: searchTerm }));
    ...
    const { mutate: handleDeleteMovie } = useMutation({
        ...
        onSuccess: () => {
            Swal.fire('Thành Công!', 'Đã xóa phim', 'success');
    ...
    const handleAddMovie = async () => {
        try {
            ...
            if (result) {
                ...
                dispatch(getMovieList({ page: 1, tenPhim: searchTerm }));
            } else {
                Swal.fire('Lỗi!', 'Có lỗi xảy ra khi thêm phim', 'error');
            }
